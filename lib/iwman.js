...
var IWMan = function (_EventEmitter) {
  ...
  function IWMan() {
    ...
    _this.mode = function (iface) {
      return _this.status(iface).then(function (status) {
    ...
    _this.isWiFiMode = function (iface) {
      return _this.mode(iface).then(function (mode) {
        return mode === 'wifi';
      });
    };
    ...
    _this.ifup = function (iface) {
      iface = iface || 'wlan0';
      var cmd = 'ifup ' + iface + ' up';
    ...
    _this.ifdown = function (iface) {
      iface = iface || 'wlan0';
      return (0, _childProcessPromise.exec)('ifdown ' + iface + ' down').then(function () {
        console.log("ifdown " + iface + " successful...");
        _this.emit('ifdown');
      });
    };

    _this.ifreboot = function (iface) {
      iface = iface || 'wlan0';
      return _this.ifdown(iface).then(function () {
        return _this.ifup(iface);
      }).then(function () {
    ...
    _this.scan = function (options) {
      ...
      var iface = typeof options === 'string' ? options : options.iface;
    ...
    _this.stopAP = function (iface) {
      return _ap2.default.stop(iface);
    };
